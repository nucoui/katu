...
export const hastToJsx = (tag: string, id: string, hast: any, children?: any, key?: string): VNode => {
  ...
  if (hast.type === "root") {
    ...
    return h(Fragment, { key }, rootChildren);
  }

  // テキストノードの場合はそのまま返す
  if (hast.type === "text") {
    return h(tag as any, { id, key }, hast.value);
  }
  ...
  if (hast.tagName === "template") {
    return h("tora-container" as any, { id, key: keyToUse }, nodeChildren);
  }
  ...
  if (hast.tagName === "style") {
    ...
    return h("style", { ...props, key: keyToUse }, transformedStyle);
  }

  // VueのJSX要素を作成
  return h(hast.tagName as any, { ...props, key: keyToUse }, nodeChildren);
};
